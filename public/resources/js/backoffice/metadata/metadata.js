...
md.init = function(translator) {
	...
	md.dataTable = ctrl.DataTable({
		dom : 'frlBtip',
		buttons : [ {
			text : ' <i class="fa fa-plus-circle" aria-hidden="true"></i> '+TRANS["metadata"],
			action : md.addMetadata
		}, {
			extend : 'excelHtml5',
...
/**
* Render data to push on colum hydro
*
* @param {string} row
* @return {string} text
*/
var renderColumnHydro = function(row){
	var hydro = JH.GetJsonValue(row, "hydroinfo");
	if(typeof hydro === undefined || hydro == null){return false}
	var text = ""
	for (var i = 0 ; i < hydro.length ; i++){
		if ( i != 0 ){ text += ", "; }
		text += "<span>" + JH.GetJsonLangValue(hydro[i], "hydroinfo_name", true) + "</span>" ;
	}
	return text;
}
...
/**
* Render button Edit and Delete data on datatable
*
* @param {string} row
* @return {string} text
*/
var renderToolButtons = function(row) {
	var s = '<i class="btn btn-edit" title="' + md.translator['btn_edit'] + '"></i>' +
	'<i class="btn btn-delete" title="' + md.translator['btn_delete'] + '"></i>';
	return s;
}
...
/**
* Generate input form for insert Frequency (not in use)
*
*/
// function addInputFrequency(h){
// 	var count_input_frequency = 1;
// 	var count_input_frequency = parseInt($('.frm_datafrequency > div > input').last().attr('data-key')); //amount
//
// 	count_input_frequency += 1;
//
// 	return '<div class="col-sm-6 col-sm-offset-3 gen_fre rm-'+count_input_frequency+' delete-text"> <input type="number" min="0" class="form-control" id="dlgEditMetadata-datafrequency_'+count_input_frequency+'" value = "0" data-key="'+count_input_frequency+'"> </div><div class="col-sm-2 rm-'+count_input_frequency+' delete-text"> <select name="frequencyunit" id="dlgEditMetadata-typetime_'+count_input_frequency+'" class="form-control clr-mr-rgt" data-key="'+count_input_frequency+'"> </select> </div> <div class="col-sm-1 icon-add rm-'+count_input_frequency+' delete-text" id="delete-text"> <a class="delete-contact" data-key="rm-'+count_input_frequency+'"><i class="fa fa-times-circle btn-remove-freq" aria-hidden="true"></i></a> </div>';
// }
