...
srvData.init = function (translator) {
	...
	self.dataTable = ctrl.DataTable({
		...
		"footerCallback": function (row, data, start, end, display) {
			...
			// Sum over all pages
			total_download_size = api.column(2).data().reduce(function (a, b) {
				return parseFloat(Math.round((intVal(a) + intVal(b)) * 100) / 100).toFixed(2);
			}, 0);
...
srvData.onClickDisplay = function () {
	...
	if (filter_form == 2) {
		...
		$("#filter_year").multiselect({ includeSelectAllOption: true });
		$("#filter_year").multiselect('rebuild');
		$("#filter_year").multiselect('updateButtonText');
	...
	if (filter_form == 2) {
		...
		for (let i = 0; i < ds.data.length; i++) {
			for (let j = 0; j < ds.data[0].data_in_agency[0].length; j++) {
				var temp = {
					agency_name: ds.data[i].agency_shortname.en,
					download_date: "",
					download_size: "",
					number_of_file: "",
					number_of_record: "",
...
srvData.genChart = function (or, typeChart) {
	...
	if (filter_form == 2) {
		...
		for (let i = 0; i < data.length; i++) {
			val = data[i].agency_shortname.en
			data_date.push(val)
		}
	} else {
