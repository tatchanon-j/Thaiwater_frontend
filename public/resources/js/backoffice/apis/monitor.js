...
srvMeta.init = function(translator) {
	...
	self.service_agent_name = 'thaiwater30/backoffice/api/agent';
...
*/
srvMeta.gen_filter_agent_name = function(sn){
	var filter_agent_name = document.getElementById('filter-agent'); //filter service element
	var agent_name = apiService.getFieldValue(sn,'data'); //data service
...
/**
*Generate option of method drodown list
*
*@param {json} service_method Json data from service.
*/
srvMeta.gen_filter_service_method = function(sn){
	var filter_service_method = document.getElementById('filter-service-method'); //filter service element
	var service_method = apiService.getFieldValue(sn,'data.service_method'); //data service
	...
	/* sort option list by alphabet */
	JH.Sort(service_method,"method_name",false , function(str){
		return str.toLowerCase();
	});

	for(var i=0; i<service_method.length; i++){
		var gen_option = document.createElement('option'); //create option element

		if(service_method[i]['method_name']){
			var text_option = service_method[i]['method_name']; //option name
		}else{
			var text_option = srvMeta.translator['untitle']; //option name without service name
		}

		var value_option = service_method[i]['id']; //value option

		gen_option.text = text_option;
		gen_option.value = value_option;
		filter_service_method.add(gen_option);

		//Display filter as multiselect
		$(filter_service_method).multiselect({includeSelectAllOption: true });
		$(filter_service_method).multiselect('rebuild');
		$(filter_service_method).multiselect('selectAll',true);
		$(filter_service_method).multiselect('updateButtonText');
	}
}
...
srvMeta.btnPreviewClick = function(){
	...
	var filter_agent = $('#filter-agent').val() //ifilter agent element
	var filter_service_method = $('#filter-service-method').val() //ifilter agent element
	frm.parsley().validate()
	if(!start_date || !filter_service || !end_date || !filter_agent || !filter_service_method){
		bootbox.alert({
	...
	param['method'] = $('#filter-service-method').val().join();
