<?php
  return[
    ...
    'page_name' =>  '雨を計算するため、ダウンロードを設定してください。',
    ...
    'label_sql' => '命令セット',
    'label_table_name' => 'テーブル名',
    'label_partition_field' => 'パーティション内の列名を確認してください。',
    'label_is_dateonly_pf' => 'カラムは、一日のパーティションを決定するために使用されています。',
    'label_last_update_field' => '列の更新',
    'label_is_hot_migration_mode' => 'ホット移行モード',
    'label_data_limit_hours' => '限られた時間（時間）',

    'label_output_filtname' => '出力ファイル名',
	'label_max_process' => 'プロセスの最大数 : ',
	'tooltip_max_process' => 'プロセスの最大数=10のデフォルトをダウンロードするために1に設定することができます。',

    'tooltip_sql' => '計算に使用されるSQLコマンドセット。',
    'tooltip_table_name' => 'テーブル名は、SQLからデータを取得するために使用されます。',
    'tooltip_partition_field' => 'ほとんどの時間でpartitionを使用して分割パーティションのフィールド名。',
    'tooltip_is_dateonly_pf' => '列のみをyesまたはnoとしてパーティションを決定するために使用されています。',
    'tooltip_last_update_field' => 'hot_migrationモード',
    'tooltip_is_hot_migration_mode' => '限定タイム（時間）。',
    'tooltip_data_limit_hours' => '限定タイム（時間）。',

    'tooltip_output_filtname' => 'ファイルの拡張子ではありません、出力ファイルに名前を付けます。',
    ...
    'tooltip_result_file' => 'メディアのダウンロードから生成されたJSONファイル名。どのLAST_MODIFIEDファイル名などのアーカイブフォルダとファイルの詳細が含まれています。データは、MPの鳥のサービスからダウンロードされた場合。メタデータ情報はに、デフォルト値を指定しない場合。 filelist.json',
    'tooltip_crontab_setting' => 'サーバー上のcrontabファイルにタイムの仕事を設定します。',
    'tooltrip_label_host' => 'ドライバメソッドの名前は、利用可能なダウンロードします。テーブルから取り出された選択オプション内のオプション。フィールド名でapi.system_setting = bof.DataIntegration.dl.DownloadDriver',
    'tooltrip_host_url' => 'URLまたはそのようhttp://thaiwater.net?timeout=10をダウンロードするために使用されるFTPの名前。または使用のftpは、以下のフォームで必要とされます。  ftp://user:password@servername (特殊文字のパスワードは、このようなでurlencode前に行わなければならないとき。 # = %23 เป็นต้น
                            後に設定することによって活性化することができるパラメータ？含めて
                            1) 接続できない場合は、ダウンロード時間を設定するためのタイムアウトは、デフォルトでは30秒です。
                            2) datareは鳥のサービスからのデータをダウンロードするために使用される。所望のパターンに一致する名前タグJSONを見つける必要があります。 あなたは正規表現を使用できますが、特定のメッセージは、そのようなでurlencodeの前に行われる必要があります。 ^ ％5E％または$24は、ドライバ方法はCPY白鳥haiipic2 SWATに利用可能です。
                            3) 対象は、タイにある場合、ドライバの方法をダウンロードするために使用ubjectは、IMAP電子メールメッセージです。それはでurlencodeする前に行う必要があります。
...
                              データセットとステップの入力ファイル（入力名:)。 ',
    ...
    'tooltip_conv_partition_field' => 'フィル・パニッチパックディパーティションで共有することに呼びかけました。',
    ...
    'tooltip_input_transform_param' => '変数定数の値',
